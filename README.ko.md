...
- Git 커밋 이력을 Code Text로 가져와 코드 변경 타이핑 애니메이션 생성
- GPT 대화 타이핑 슬라이드 생성
...
- [ ] Code Text 타이핑과 GPT 대화 슬라이드에 사용자 지정 스타일 적용
...
5. 필요한 경우 효과음, 배경음, Code Text 타이핑, GPT 대화 슬라이드를 추가합니다.
6. PNG 또는 MP4로 내보냅니다.
...
번역은 슬라이드 목록 하단에서 일반 슬라이드에만 실행할 수 있습니다. 선택한 슬라이드의 텍스트 상자와 노트만 바꾸며, GPT 대화 슬라이드는 건너뜁니다.
...
- 릴리스 ZIP: `release/Slide-Cut-v1.1.5-macos-arm64.zip`
- SHA256 체크섬: `release/Slide-Cut-v1.1.5-macos-arm64.zip.sha256`
...
sequenceDiagram
  ...
  Frontend->>Frontend: Code Text 타이핑과 GPT 동적 애니메이션 프레임 렌더링
...
- `rendering.js`: 텍스트, GPT 동적 슬라이드, 내보내기 프레임용 캔버스 렌더링
